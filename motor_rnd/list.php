...
var motordetailNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {			
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('motordetailNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var motordetailNumber = dataTable.page.info().page + 1;
        setCookie('motordetailNumber', motordetailNumber, 10); // 쿠키에 페이지 번호 저장
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('motordetailNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
...
function redirectToView(num, tablename) {
    var page = motordetailNumber; // 현재 페이지 번호 (+1을 해서 1부터 시작하도록 조정)
    	
    var url = "view.php?num=" + num + "&tablename=" + tablename;          

	customPopup(url, '모터개발일지', 1200, 900); 		    
}
...
$(document).ready(function(){
	...
	$("#writeBtn").click(function(){ 
		...
		customPopup(url, '모터개발일지', 1300, 850); 	
	 });			 
