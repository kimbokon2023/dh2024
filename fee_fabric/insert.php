...
$fieldarr = [
    'item ', 'originalcost ', 'price ' , 'is_dc', 'itemcode','model', 'yuan'
];
   ...
   $update_log = date("Y-m-d H:i:s") . " - " . $_SESSION["name"] . " " . $update_log . "&#10";
	...
	try {
		...
		$sql = "UPDATE " . $DB . "." . $tablename . " SET "
			 . "basicdate = ? , item = ?, originalcost = ?, price = ?, memo = ?, update_log = ?, searchtag = ?, is_dc=?, itemcode=?, model=? , yuan=? "
			 . " WHERE num = ?";
		...
		$stmh->bindValue(10, json_encode($model, JSON_UNESCAPED_UNICODE), PDO::PARAM_STR);
		$stmh->bindValue(11, json_encode($yuan, JSON_UNESCAPED_UNICODE), PDO::PARAM_STR);
		$stmh->bindValue(12, $num, PDO::PARAM_INT);
	...
	try {
		$pdo->beginTransaction();   
		$sql = "INSERT INTO " . $DB . "." . $tablename . " (basicdate, item, originalcost, price, memo, update_log, searchtag, is_dc, itemcode, model, yuan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
