...
// 부자재에 대한 배열 가져오기
$sql = "select * from {$DB}.{$tablename} order by basicdate desc ";
...
<div class="container-fluid">  
	...
	<div class="d-flex  p-1 m-1 mt-1 justify-content-center align-items-center "> 		
		   <h5>  <?=$title_message?> </h5>	
            ...
            <small class="ms-5 text-muted"> 원단별 단가를 추가하거나 수정합니다. </small>  
		</div>	
...
<table class="table table-hover" id="myTable">
  <thead class="table-primary">
    <tr>
		  ...
		  <th class="text-center" style="width:400px;"> 내용</th>      
		  <th class="text-center" style="width:500px;"> 메모</th>      
    </tr>        
  </thead>	  
  <tbody>
<?php
...
var feefabricpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {			
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('feefabricpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var feefabricpageNumber = dataTable.page.info().page + 1;
        setCookie('feefabricpageNumber', feefabricpageNumber, 10); // 쿠키에 페이지 번호 저장
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('feefabricpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
