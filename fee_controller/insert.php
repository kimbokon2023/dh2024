   ...
   $update_log = date("Y-m-d H:i:s") . " - " . $_SESSION["name"] . " " . $update_log . "&#10";
	...
	try {
		...
		$sql = "UPDATE " . $DB . "." . $tablename . " SET "
			 . "basicdate = ? , item = ?, originalcost = ?, price = ?, memo = ?, update_log = ?, searchtag = ? , yuan = ? "
			 . " WHERE num = ?";
		...
		$stmh->bindValue(8, json_encode($yuan, JSON_UNESCAPED_UNICODE), PDO::PARAM_STR);
	...
	try {
		$pdo->beginTransaction();   
		$sql = "INSERT INTO " . $DB . "." . $tablename . " (basicdate, item, originalcost, price, memo, update_log, searchtag, yuan ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		...
		$stmh->bindValue(6, $update_log, PDO::PARAM_STR);		
		$stmh->bindValue(7, $searchtag, PDO::PARAM_STR);		
		$stmh->bindValue(8, json_encode($yuan), PDO::PARAM_STR);
