...
// 첫 화면 표시 문구
$title_message = '결선/AS 비용 미입금(청구 후 미입금)'; 
?>
...
if (checkNull($search)) {
	$sql = "SELECT * FROM ".$DB.".".$tablename." 
        WHERE searchtag LIKE '%$search%' AND is_deleted IS NULL AND payment='paid' And paydate IS NULL And demandDate != '0000-00-00' " . $order_by;	
} else {
	$sql = "SELECT * FROM ".$DB.".".$tablename." WHERE is_deleted IS NULL AND payment='paid' And paydate IS NULL  And demandDate != '0000-00-00' " . $order_by;
}
...
<div class="card justify-content-center text-center mt-5">
	...
	<div class="card-header d-flex justify-content-center align-items-center">
		...
		<small class="ms-5 text-muted">결선/AS 완료현장 (유상건) 청구완료 후 미입금 업체 확인</small>  
	</div>
	<div class="card-body">								
		...
		<div class="table-responsive">	
			<table class="table table-hover" id="myTable">		 
				<thead class="table-success">
					...
					<th class="text-center w100px">청구일</th>					
					<th class="text-center w100px">AS완료일</th>					
					<th class="text-center w150px">요청업체</th>
					<th class="text-center w80px">요청인</th>
					<th class="text-center w120px">비용부담업체</th>
					<th class="text-center w80px">비용</th>
					<th class="text-center w150px">현장주소</th>					
					<th class="text-center w150px">처리내용</th>					
					<th class="text-center w250px">처리결과(구체적)</th>					
				</thead>
				<tbody>		      	 
				<?php  		
				$start_num = $total_row;  			    
				...
				<tr onclick="redirectToView('<?=$num?>')">  
					...
					<td class="text-center"><?= $demandDate == '0000-00-00' ? '' : $demandDate ?></td>
