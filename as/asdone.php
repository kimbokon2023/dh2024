...
// 첫 화면 표시 문구
$title_message = '결선/AS 완료 현장 (무상완료/유상 입금완료)'; 
?>
...
if (checkNull($search)) {
	$sql = "SELECT * FROM ".$DB.".".$tablename." 
        WHERE searchtag LIKE '%$search%' AND is_deleted IS NULL AND ((payment='paid' And paydate IS NOT NULL) or (payment='free' And asendday IS NOT NULL))  " . $order_by;	
} else {
	$sql = "SELECT * FROM ".$DB.".".$tablename." WHERE is_deleted IS NULL AND ((payment='paid' And paydate IS NOT NULL) or (payment='free' And asendday IS NOT NULL)) " . $order_by;
}
