<?php
$orderdate = isset($_REQUEST['orderdate']) ? $_REQUEST['orderdate'] : '';
$towhom = isset($_REQUEST['towhom']) ? $_REQUEST['towhom'] : '';
$reply = isset($_REQUEST['reply']) ? $_REQUEST['reply'] : '';
$deadline = isset($_REQUEST['deadline']) ? $_REQUEST['deadline'] : '';
$work_status = isset($_REQUEST['work_status']) ? $_REQUEST['work_status'] : '';
$title = isset($_REQUEST['title']) ? $_REQUEST['title'] : '';
$title_after = isset($_REQUEST['title_after']) ? $_REQUEST['title_after'] : '';
$first_writer = isset($_REQUEST['first_writer']) ? $_REQUEST['first_writer'] : '';
$update_log = isset($_REQUEST['update_log']) ? $_REQUEST['update_log'] : '';
$searchtag = isset($_REQUEST['searchtag']) ? $_REQUEST['searchtag'] : '';
?>
