 ...
 $success = true;
  ...
  if ($mode == "modify") {
    ...
    try {
        ...
        $sql = "update " . $DB . "." . $tablename . " set subject=?, content=?, is_html=?, suggestioncheck=?, searchtext=? where num=?";
        $stmh = $pdo->prepare($sql);
        $stmh->bindValue(1, $subject, PDO::PARAM_STR);
        $stmh->bindValue(2, $content, PDO::PARAM_LOB);
        $stmh->bindValue(3, $is_html, PDO::PARAM_STR);
        $stmh->bindValue(4, $suggestioncheck, PDO::PARAM_STR);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "insert into " . $DB . "." . $tablename . " (id, name, subject, content, regist_day, hit, is_html, suggestioncheck, searchtext) ";
        $sql .= "values(?, ?, ?, ?, now(), 0, ?, ?, ?)";
        $stmh = $pdo->prepare($sql);
        ...
        $stmh->bindValue(6, $suggestioncheck, PDO::PARAM_STR);
