				 ...
				 <div class="row"> 
					 <div class="col-3-sm"> 
						<div class="d-flex justify-content-center align-items-center"> 							 
							...
							<input type="checkbox" name="suggestioncheck" value="y" <?php if($suggestioncheck=='y') print 'checked'; ?> > &nbsp; 중요건의 										
						</div>
					</div>
					 <div class="col-5-sm"> 													
						<div class="d-flex  mt-2 justify-content-center align-items-center"> 							 
							제목 &nbsp; 					
							<input id="subject" name="subject" type="text" required class="form-control" style="width:500px;" autocomplete="off" <?php if($mode=="modify"){ ?> value="<?=$subject?>" <?php }?>>&nbsp;														
						</div>	
					</div>
				</div>
				</div>
...
var ajaxRequest_suggestion = null;

$(document).ready(function(){	

      $('#summernote').summernote({
        placeholder: '(제품 품질개선 해야 할 내용이나 회사에 대한 건의사항이 있으면 부담갖지 말고 올려 주세요) ',
	...
	function Fninsert() {	 
		...
		// }	
	   // console.log(data);   
		if (ajaxRequest_suggestion !== null) {
			ajaxRequest_suggestion.abort();
		}	
	   
		ajaxRequest_suggestion = $.ajax({
			enctype: 'multipart/form-data',    // file을 서버에 전송하려면 이렇게 해야 함 주의
			processData: false,    
			contentType: false,      
			cache: false,           
