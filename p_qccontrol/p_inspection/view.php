 ...
 try{
	...
	$item_content = str_replace("\n", "<br>", $item_content);
     ...
     try{
       ...
       $sql = "update mirae8440." . $DB . " set hit=? where num=?";   // 글 조회수 증가
       $stmh = $pdo->prepare($sql);  
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<button type="button"   class="btn btn-outline-dark  btn-sm" onclick="location.href='list.php?page=<?=$page?>'" > 목록(List) </button>	&nbsp;
<?php
...
if($_SESSION["userid"]==$item_id || $_SESSION["userid"]=="admin" ||
	...
	{
?>			
		<button type="button"   class="btn btn-outline-secondary btn-sm" onclick="location.href='write_form.php?mode=modify&num=<?=$num?>&page=<?=$page?>&search=<?=$search?>&Bigsearch=<?=$Bigsearch?>&find=<?=$find?>&year=<?=$year?>&search=<?=$search?>&process=<?=$process?>&asprocess=<?=$asprocess?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&separate_date=<?=$separate_date?>&DB=<?=$DB?>'" > 수정 </button> &nbsp;			
		<button type="button"   class="btn btn-outline-danger btn-sm" onclick="javascript:del('delete.php?num=<?=$num?>&page=<?=$page?>&DB=<?=$DB?>')" > 삭제 </button>	 &nbsp;
<?php  	}    ?>		
  ...
  <div class="row d-flex  p-2 m-2 mt-1 mb-1 justify-content-center bg-secondary text-white"> 		
		  ...
		  <div class="col-7" > QC 공정표명 : &nbsp; <?= $item_subject ?> </div>
		  <div class="col-5" > <?= $noticecheck_memo ?> |<?= $item_nick ?> | 조회 : <?= $item_hit ?> | <?= $item_date ?>   </div>
