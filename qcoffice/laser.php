...
if($selnum==1)
{	
	$sql="select * from " . $DB . ".myarealist where item='" . $mcno . "' " . $a; 						
}  
 
if($selnum==2)
{	
	$sql="select * from " . $DB . ".myarealist where item='" . $mcno . "' and term='주간' " . $a; 						
}  
if($selnum==3) 
{	
	$sql="select * from " . $DB . ".myarealist where item='" . $mcno . "' and term='1개월' " . $a; 						
}  
if($selnum==4) 
{	
	$sql="select * from " . $DB . ".myarealist where item='" . $mcno . "' and term='2개월' " . $a; 						
}  
if($selnum==5) 
{	
	$sql="select * from " . $DB . ".myarealist where item='" . $mcno . "' and term='6개월' " . $a; 						
}  
// 미점검 리스트
if($selnum==6) 
{	 
  ...
  for($k=0; $k<count($period); $k++)   // 주간/1개월/2개월/6개월  4개항목 반복문
	{
	  ...
	  for($j=1; $j<=(int)$arrtmp[$k]; $j++)
	  {
	   ...
	   if($k==count($period)-1)
		...
		$sql="select * from " . $DB . ".myarealist where " . $sqladd . $a; 					
	...
	for($j=0;$j<count($mcno_arr);$j++)
	 ...
	 // var_dump('배열점검');
	  ...
	  for($j=1; $j<=(int)$arrtmp[0]; $j++)
	  {
		...
		$sql="select * from " . $DB . ".myarealist where (item='" . $mcno . "'" . $sqladd . $a; 							
...
function redirectToView(num) {       	
	...
	customPopup(url, title, 1300, 800); 		    
}
