...
$startDate = "$year-$prevMonth-01";
$endDate = date("Y-m-t", strtotime($startDate));


// echo '<pre> 이전달 startDate';
// print_r($startDate);
// echo '</pre>';

// echo '<pre> 이전달 endDate';
// print_r($endDate);
// echo '</pre>';
...
// 수입 내역 조회
$incomeSql =  "
    SELECT SUM(sales) as totalAmount
    FROM {$DB}.monthly_sales
    WHERE invoice_issued = '발행'
    AND closure_date BETWEEN :startDate AND :endDate
    AND (is_deleted = '0' or is_deleted IS NULL) 
    GROUP BY invoice_issued
";
...
$expenseSql = "
    SELECT num, content, amount, registDate
    FROM {$DB}.$tablename
    WHERE inoutsep = '지출'
    AND registDate BETWEEN :startDate AND :endDate
    AND (is_deleted = '0' or is_deleted IS NULL)
            ...
            <table class="table table-hover" id="detailTable">
				...
				<tbody>
					...
					// echo '</pre>';						
						...
						// For income section
						if ($i == 0) {
							echo '<tr><td class="text-center fw-bold">계산서발행 총액</td>';
						} else {
				...
				<tfoot class="table-secondary">
					...
					<tr>
						<th class="text-end" colspan="3"> 차액 &nbsp; </th>
...
function loadForm(mode, num = null) {
    ...
    ajaxRequest = $.ajax({
        type: "POST",
        url: "fetch_modal.php",
        data: { mode: mode, num: num, tablename : $("#tablename").val() },
        dataType: "html",
...
function loadSpecificMonth(month) {
    const year = document.getElementById('year').value;
    window.location.href = `list.php?year=${year}&startMonth=${month}&endMonth=${month}`;
}
</script>
