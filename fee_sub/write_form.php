...
if($mode === 'copy')
	$title_message = "(데이터복사) DH모터 부자재 (원가 및 단가)" ;
else
...
<div class="container-fluid"> 
		...
		<div class="row d-flex justify-content-center align-items-center ">	        
			...
			<div class="card-body text-center">			
				...
				<h3>	<?=$title_message?>    </h3>   &nbsp; &nbsp;  &nbsp; &nbsp; 	
					<?php if($mode =='view') { ?>		
					<button type="button" class="btn btn-dark btn-sm me-1" onclick="location.href='write_form.php?mode=modify&num=<?=$num?>&tablename=<?=$tablename?>';" >  <i class="bi bi-pencil-square"></i>   전체 수정 </button>
					<?php } ?>		
				   <?php if($mode!=='view') { ?>
					...
					<table class="table table-hover" id="myTable">
					  <thead class="table-primary">
						<tr>
						  ...
						  <th class="text-center " style="width:250px;">품목</th>      						  
...
$(document).ready(function(){		  
    ...
    for (let i = 0; i < row_count; i += 1) {
        ...
        let itemcode = '';
        if (item[i] && item[i].includes('콘트롤박스') && (item[i].includes('220') || item[i].includes('380'))) {
            let parts = item[i].match(/(\d+V) \[(.*?)\] 콘트롤박스\((.*?)\)/);
            if (parts) {
                itemcode += parts[1].slice(0, -1) + '-'; // Add voltage without 'V'
                itemcode += parts[2] + '-'; // Add wire type
                itemcode += '콘트롤박스-'; // Add item type
                itemcode += parts[3].replace(/k/g, '').replace(/~/g, '-').replace(/\(|\)/g, ''); // Add range without 'k' and replace '~' with '-'
            }
        }
...
$(document).ready(function(){	
   ...
   hideOverlay();	
	...
	// 삽입/수정하는 모듈 
	function Fninsert(datanumber = null) { 
...
window.onload = function() {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        allData.yuan.push(cells[5].innerText.trim().replace(/,/g, ''));
    }
};
</script>
