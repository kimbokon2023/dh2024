...
if($mode === 'copy')
	$title_message = "(데이터복사) DH모터 원가 및 단가" ;
else
	$title_message = "DH모터 원가 및 단가" ;
 ?>
...
<input type="hidden" id="wire" name="wire"  >
<input type="hidden" id="volt" name="volt"  >
<input type="hidden" id="unit" name="unit"  >
<input type="hidden" id="upweight" name="upweight"  >
<input type="hidden" id="price" name="price"  >
<input type="hidden" id="originalcost" name="originalcost"  >
<input type="hidden" id="item" name="item"  >
<input type="hidden" id="ecountcode" name="ecountcode"  >
	...
	<div class="d-flex p-1 mb-1  justify-content-center align-items-center ">					  
		<span class="form-control mb-2" style="width:90%;">
			<div class="d-flex p-1 mb-1  justify-content-center align-items-center ">
				...
				<textarea name="memo" id="memo" class="form-control" placeholder="(메모)" style="width:700px;height:70px;"><?=$memo?></textarea>				    								 
			</div> 
		</span>
	</div>
			...
			<table class="table" id="myTable">
			  <thead class="table-primary">
				<tr>
				  ...
				  <th class="text-center "style="width:200px;">품목</th>      
			...
			<tbody>
				...
				try {
					...
					foreach ($rows as $row) {
						...
						$volt = json_decode($row['volt'], true) ?? [];
						$wire = json_decode($row['wire'], true) ?? [];
						...
						for ($i = 0; $i < $minSize; $i++) {
							...
							<tr>
								...
								<td class="text-center"><?= htmlspecialchars($volt[$i] ?? '') ?></td>
								<td class="text-center"><?= htmlspecialchars($wire[$i] ?? '') ?></td>
								<td class="text-center"><?= htmlspecialchars($item[$i] ?? '') ?></td>
								<td class="text-center"><?= htmlspecialchars($upweight[$i] ?? '') ?></td>
								<td class="text-center"><?= htmlspecialchars($unit[$i] ?? '') ?></td>
								<td class="text-end"><?= htmlspecialchars(number_format($originalcost[$i] ?? 0)) ?></td>
								<td class="text-end"><?= htmlspecialchars(number_format($price[$i] ?? 0)) ?></td>
								<td class="text-center"><?= htmlspecialchars($ecountcode[$i] ?? '') ?></td>
...
$(document).ready(function(){
	var wire         = <?php echo json_encode(isset($wire) ? json_decode($wire) : []); ?> || [];
	var volt         = <?php echo json_encode(isset($volt) ? json_decode($volt) : []); ?> || [];
	var unit         = <?php echo json_encode(isset($unit) ? json_decode($unit) : []); ?> || [];
	var upweight     = <?php echo json_encode(isset($upweight) ? json_decode($upweight) : []); ?> || [];
	var price        = <?php echo json_encode(isset($price) ? json_decode($price) : []); ?> || [];
	var originalcost = <?php echo json_encode(isset($originalcost) ? json_decode($originalcost) : []); ?> || [];
	var item         = <?php echo json_encode(isset($item) ? json_decode($item) : []); ?> || [];
	var ecountcode   = <?php echo json_encode(isset($ecountcode) ? json_decode($ecountcode) : []); ?> || [];
	...
	for (let i = 0; i < row_count; i += 1) {
		const row = { name: i };
		row[`wire`]         = wire[i] !== undefined ? wire[i] : "";
		row[`volt`]         = volt[i] !== undefined ? volt[i] : "";
		row[`unit`]         = unit[i] !== undefined ? unit[i] : "";
		row[`upweight`]     = upweight[i] !== undefined ? upweight[i] : "";
		row[`item`]         = item[i] !== undefined ? item[i] : "";
		row[`price`]        = price[i] !== undefined ? price[i] : "";
		...
		// ecountcode generation logic
		if (unit[i] !== '브라켓트') {
			let ecountcode = '';
			if (volt[i] && (volt[i].startsWith('220') || volt[i].startsWith('380'))) {
				ecountcode += volt[i] + '-';
			}
			if (wire[i]) {
				ecountcode += wire[i] + '-';
			}
			if (item[i] == '무기둥모터') {  // 무기둥모터 품목추가
				ecountcode += item[i] + '-';
			}				
			if (upweight[i]) {
				ecountcode += upweight[i].replace(/k|K/g, '') + '-';
			}
			...
			row[`ecountcode`] = ecountcode;
		} else {			
			// row[`ecountcode`] = item[i] + (upweight[i].trim() !== '' ? '-' + upweight[i] : upweight[i]);
			row[`ecountcode`] = ecountcode[i] ;
		}
...
function savegrid() {
    ...
    let volt  =  new Array();  
    let unit  =  new Array();  
    let upweight  =  new Array();  
    let price  =  new Array();  
    let originalcost  =  new Array();  
    let item  =  new Array();     
    let ecountcode  =  new Array();     
    let yuan  =  new Array();     
    
    const MAXcount = grid.getRowCount();
    for (let i = 0; i < MAXcount; i++) {
        wire.push(grid.getValue(i, 'wire'));
        volt.push(grid.getValue(i, 'volt'));
        unit.push(grid.getValue(i, 'unit'));
        upweight.push(grid.getValue(i, 'upweight'));
        ...
        ecountcode.push(grid.getValue(i, 'ecountcode'));
        yuan.push(grid.getValue(i, 'yuan'));
    }

    // Set the value of hidden inputs
    document.getElementById('wire').value = JSON.stringify(wire);
    document.getElementById('volt').value = JSON.stringify(volt);
    document.getElementById('unit').value = JSON.stringify(unit);
    document.getElementById('upweight').value = JSON.stringify(upweight);
    ...
    document.getElementById('ecountcode').value = JSON.stringify(ecountcode);
 ...
 function deleteRow(index=null) {		
	 ...
	 // 화면 지워준다
	  for(i = 0; i < MAXcount ; i++) {				
			grid.setValue(i, 'wire', '');
			grid.setValue(i, 'volt', '');
			grid.setValue(i, 'unit', '');
			grid.setValue(i, 'upweight', '');
			grid.setValue(i, 'price', '');
			grid.setValue(i, 'originalcost', '');
			grid.setValue(i, 'item', '');					
			grid.setValue(i, 'ecountcode', '');					
	  ...
	  // 새로 삽입되는 행을 포함한 데이터를 새로 넣어준다
		for(i = 0; i < wire.length ; i++) {				
				grid.setValue(i, 'wire', wire[i]);
				grid.setValue(i, 'volt', volt[i]);
				grid.setValue(i, 'unit', unit[i]);
				grid.setValue(i, 'upweight', upweight[i]);
				grid.setValue(i, 'price', price[i]);
				grid.setValue(i, 'originalcost', originalcost[i]);
				grid.setValue(i, 'item', item[i]);					
				grid.setValue(i, 'ecountcode', ecountcode[i]);					
