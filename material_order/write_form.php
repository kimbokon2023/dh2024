...
<form id="board_form" name="board_form" method="post" enctype="multipart/form-data" onkeydown="return captureReturnKey(event)">
 ...
 <?php
	function generateTableSection($id, $title, $badgeClass = 'bg-secondary') {
		...
		<div class='d-flex row justify-content-center m-1 p-2 rounded' style='border: 1px solid #392f31;'>
			<div class='d-flex mb-2'>
				...
				<button type='button' class='btn btn-dark btn-sm viewNoBtn add-row' data-table='{$id}Table' style='margin-right: 5px; padding:5px;'>+</button>
			</div>
	...
	generateTableSection('motorlist', ' 모터');
	generateTableSection('wirelessClist', '무선 콘트롤러');
	generateTableSection('wireClist', '유선 콘트롤러');
	generateTableSection('wirelessLinklist', '무선 제어기');
	generateTableSection('wireLinklist', '유선 제어기');
	generateTableSection('bracketlist', '브라켓트');
	?>
</div>
</form>
