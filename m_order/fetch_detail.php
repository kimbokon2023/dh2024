...
try {
	...
	foreach (array_reverse($all_rows) as $row) {
		...
		// $totalOut 값이 있을 때만 onclick 속성을 추가
		$onclickAttribute = $totalOut !== '' ? ' onclick="redirectToView('.htmlspecialchars($row['num']).', \'motor\');"' : '';

		// $totalIn 값이 존재할 때  클래스를 추가
		$rowClass = $totalIn !== '' ? 'text-danger' : '';

		echo '<tr data-num="'.htmlspecialchars($row['num']).'"'.$onclickAttribute.'>';
		...
		echo '<td class="text-center ' . $rowClass. ' fw-bold">'.$totalIn.'</td>';
		echo '<td class="text-center ' . $rowClass. '">'.$totalOut.'</td>';
		echo '<td class="text-center ' . $rowClass. '">'.htmlspecialchars($row['stock']).'</td>';
