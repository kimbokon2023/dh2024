...
$pdo = db_connect();
 ...
 try{
	 ...
	 if($suggestioncheck=='y')
		 $suggestioncheck_memo='(중요회의)';
	    else
			$suggestioncheck_memo='';
...
$admin = isset($_SESSION["level"]) ? $_SESSION["level"] : '';
		...
		<div class="card">  
			<div class="card-body">  	 
				<div class="row d-flex  p-2 m-2 mt-1 mb-1 justify-content-center bg-secondary text-white align-items-center"> 		   
				  <div class="col-7 text-start fw-bold fs-6" > [<?= $registration_date ?>] <?= $subject ?> </div>
				  <div class="col-5 text-end" > <?= $suggestioncheck_memo ?> |<?= $name ?> | 조회 : <?= $hit ?> | <?= $regist_day ?>   </div>   
				</div>	
