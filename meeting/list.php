  ...
  if(isset($_REQUEST["mode"]))
   ...
   if($mode=="search"){
         if(!$search) {
              ...
              $sql="select * from " . $DB . "." . $tablename . " where name like '%$search%' or subject like '%$search%'  or regist_day like '%$search%' or registration_date like '%$search%' or searchtext like '%$search%'  order by num desc ";              
       } else {
              $sql="select * from " . $DB . "." . $tablename . " order  by num desc";              
       }
		...
		<div class="card-body">
			...
			<div class="d-flex p-0 justify-content-center">
				<div class="alert alert-info" role="alert" style="font-size: 14px;">
					  회의 내용을 기록한 자료입니다. 전체 일정표에서 일자별 회의록을 볼 수 있습니다.
				</div>
			</div>
