...
$suggestioncheck = $_REQUEST["suggestioncheck"] ?? "";
$registration_date = $_REQUEST["registration_date"] ?? date("Y-m-d"); // 기본값은 오늘 날짜
...
<div class="container-fluid">  
				 ...
				 <div class="row"> 
					 <div class="col-3-sm"> 
						<div class="d-flex justify-content-center align-items-center"> 							 
							...
							<input type="checkbox" name="suggestioncheck" value="y" <?php if($suggestioncheck=='y') print 'checked'; ?> > &nbsp; 중요회의 										
						</div>
					</div>
					 <div class="col-5-sm"> 													
						<div class="d-flex  mt-2 justify-content-center align-items-center"> 							                             
							<label for="registration_date" class="form-label">회의일자</label> &nbsp;					
							<input id="registration_date" name="registration_date" type="date" required class="form-control" style="width:120px;" value="<?=$registration_date?>">&nbsp;
							<label for="subject" class="form-label"></label>제목</label> &nbsp; 					
							<input id="subject" name="subject" type="text" required class="form-control" style="width:350px;" autocomplete="off"  value="<?=$subject?>" >&nbsp;														
						</div>	
					</div>
				</div>
				</div>
	...
	<div class="d-flex justify-content-start align-items-center w-100"> 		
		<textarea id="summernote" name="content" rows="20" required style="width: 100%;"><?=$content?></textarea>
	</div>
...
var ajaxRequest_meeting = null;

$(document).ready(function(){	

      $('#summernote').summernote({
        placeholder: '(회의 내용과 결정사항을 기록해주세요. 전체 일정표에서 일자별 회의록을 볼 수 있습니다.) ',
			...
			if (result.isConfirmed) {
				$.ajax({
					...
					dataType: 'json',
					  ...
					  // 파일 삭제 
						$("#file" + divID).remove();  // 파일요소 삭제
						$("#delPic" + divID).remove();  // 삭제버튼 요소 삭제
						$("#pInput").val('');					
			...
			if (result.isConfirmed) {
				$.ajax({
					...
					dataType: 'json',
					  ...
					  // 이미지 삭제 
						$("#image" + divID).remove();  // 이미지요소 삭제
						$("#delPicimage" + divID).remove();  // 삭제버튼 요소 삭제
						$("#pInput").val('');	
	...
	function displayimageLoad() {    
		...
		for(i=0;i<saveimagename_arr.length;i++) {
				   ...
				   $("#displayimage").append("&nbsp;<button type='button' class='btn btn-outline-danger btn-sm' id='delPicimage" + i + "' onclick=delPicimageFn('" + i + "','" +  saveimagename_arr[i] + "')> 삭제 </button>&nbsp; <br>");					   
		  }		   
			$("#pInput").val('');	
	}
	...
	function Fninsert() {	 
		...
		// }	
	   // console.log(data);   
		if (ajaxRequest_meeting !== null) {
			ajaxRequest_meeting.abort();
		}	
	   
		ajaxRequest_meeting = $.ajax({
			enctype: 'multipart/form-data',    // file을 서버에 전송하려면 이렇게 해야 함 주의
			processData: false,    
			contentType: false,      
			cache: false,           
