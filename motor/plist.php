...
$company = $_SESSION['secondordnum'];

// echo $company;

// 첫 화면 표시 문구
$title_message = '(발주처용) DH 모터 수주'; 
 ?>
...
$common= $SettingDate . " between '$fromdate' and '$Transtodate' and is_deleted IS NULL and secondordnum = {$company} ";
...
<form id="board_form" name="board_form" method="post" action="plist.php?mode=search">  
...
<div class="container-fluid">  
	...
	<div class="d-flex  p-1 m-1 mt-1 mb-1 justify-content-center align-items-center"> 	  
		...
		<select id="find" name="find" class="form-control" style="width:70px;">
			<?php
			$options = array(
				'all' => '전체',
				'workplacename' => '현장명',				
...
<table class="table table-hover" id="myTable">
  <thead class="table-primary">
    <tr>
      ...
      <th class="text-center " style="width:100px;" >접수</th>
      <th class="text-center " style="width:80px;">납기예정</th>
      <th class="text-center " style="width:50px;">출고</th>
      <th class="text-center " style="width:50px;">청구</th>      	  
      <th id="showstatus" class="text-center" style="width:100px;"> 진행사항 </th>		  
	  <th class="text-center text-danger" style="width:80px;"> <i class="bi bi-image"></i> 출하  </th>		  
	  ...
	  <th class="text-center " style="width:100px;">배송방법</th>    
	  <th class="text-center " style="width:150px;">배송지</th>
...
try {
    ...
    foreach ($rows as $row) {
		...
		if($deadline == '0000-00-00')
			$deadline = '';
		if($outputdate == '0000-00-00')
			$outputdate = '';
		if($demand == '0000-00-00')
			$demand = '';		
	  ...
	  $sqltmp=" select * from ".$DB.".picuploads where parentnum ='$num'";		  
        ...
        if (strpos($deliverymethod, '화물') !== false || strpos($deliverymethod, '택배') !== false)
		{
			// 상차지에 화물지점표기함
       ...
       if($deliverymethod == '직배송' )
        ...
        <tr onclick="redirectToView('<?= $row['num'] ?>', '<?= $tablename ?>')">
            ...
            <td class="text-center"><?=iconv_substr($deadline, 5, 5, "utf-8")?></td>
            <td class="text-center"><?=iconv_substr($outputdate, 5, 5, "utf-8")?></td>
            <td class="text-center"><?=iconv_substr($demand, 5, 5, "utf-8")?></td>
			...
			<td class="text-center fw-bold"><?=$registpicture?></td>			
            <td class="text-start"><?= $row['secondord'] ?></td>			
			...
			<?php
				if($deliverymethod  == '대신화물')
					...
					print ' <td class="text-center align-middle">' .  $deliverymethod . '[' . $delcompany . '] </td>';			
				  else
...
function redirectToView(num, tablename) {	
    var url = "write_form_sub.php?mode=view&num=" + num + "&tablename=" + tablename;          
...
$(document).ready(function(){	
	$("#writeBtn").click(function(){ 		
		...
		var url = "write_form_sub.php?tablename=" + tablename; 				
		customPopup(url, '수주내역', 1850, 900); 	
	 });			 
